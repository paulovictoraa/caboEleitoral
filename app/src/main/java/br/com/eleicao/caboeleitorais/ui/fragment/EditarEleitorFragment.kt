...
class PagamentoFragment : BaseFragment() {
    ...
    private fun buscaEleitor() {
        ...
        viewModel.buscaEleitorPorId(eleitorId).observe(viewLifecycleOwner, Observer {
            it?.let {
                eleitorEscolhido = it
                ...
                nascimento.text = Editable.Factory.getInstance().newEditable(it.dataNascimento)
                colegio.text = Editable.Factory.getInstance().newEditable(it.colegioDeVotacao)
